...
def create_app():
    ...
    with app.app_context():
        ...
        for sql in [
            ...
            "CREATE TABLE IF NOT EXISTS restore_step_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, restore_task_id INTEGER NOT NULL REFERENCES restore_tasks(id), step_key VARCHAR(50) NOT NULL, label VARCHAR(100) NOT NULL, status VARCHAR(20) NOT NULL DEFAULT 'running', detail TEXT DEFAULT '', metrics_json TEXT DEFAULT '{}', exit_code INTEGER, stdout_tail TEXT DEFAULT '', stderr_tail TEXT DEFAULT '', remote_job_dir VARCHAR(1000) DEFAULT '', elapsed_ms BIGINT DEFAULT 0, started_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at DATETIME)",
            "CREATE INDEX IF NOT EXISTS ix_restore_step_logs_task_id ON restore_step_logs (restore_task_id)",
            ...
            "CREATE TABLE IF NOT EXISTS mirror_audit_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, mirror_config_id INTEGER NOT NULL REFERENCES mirror_configs(id), repo_name VARCHAR(300) NOT NULL DEFAULT '', action VARCHAR(50) NOT NULL, reason TEXT DEFAULT '', status VARCHAR(20) NOT NULL DEFAULT 'success', detail TEXT DEFAULT '', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
            "ALTER TABLE mirror_configs ADD COLUMN sync_on_commit BOOLEAN DEFAULT 1",
            "ALTER TABLE mirror_configs ADD COLUMN progress_stage VARCHAR(50) DEFAULT ''",
            "ALTER TABLE mirror_configs ADD COLUMN progress_label VARCHAR(100) DEFAULT ''",
            "ALTER TABLE mirror_configs ADD COLUMN progress_percent INTEGER DEFAULT 0",
            "ALTER TABLE mirror_configs ADD COLUMN progress_detail TEXT DEFAULT ''",
            "ALTER TABLE mirror_configs ADD COLUMN progress_updated_at DATETIME",
            "ALTER TABLE mirror_configs ADD COLUMN current_repo_name VARCHAR(300) DEFAULT ''",
            "ALTER TABLE mirror_configs ADD COLUMN current_repo_index INTEGER DEFAULT 0",
            "ALTER TABLE mirror_configs ADD COLUMN current_repo_total INTEGER DEFAULT 0",
    ...
    def check_host_ip():
        if not request.endpoint:
            return None
        read_only = {'list_servers', 'get_server', 'list_backups', 'list_restore_tasks', 'get_restore_steps', 'get_schedule_logs', 'list_schedules', 'list_alerts', 'alert_summary', 'recent_activity', 'list_mirrors', 'mirror_status', 'overview', 'commit_trend', 'repo_ranking', 'author_ranking', 'author_detail', 'author_repos', 'author_trend', 'delete_info', 'list_rules', 'list_repos', 'test_rule'}
