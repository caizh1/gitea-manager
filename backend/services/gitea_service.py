...
def _backup_local(server, backup):
    container_tmp = f'/tmp/{backup.filename}'
    local_path = os.path.join(BACKUP_DIR, backup.filename)
    os.makedirs(BACKUP_DIR, exist_ok=True)

    dump_cmd = f'gitea dump -c /data/gitea/conf/app.ini -f {container_tmp}'
    logging.info('[备份-本地] 执行: gitea dump ...')
    exit_code, out = local_exec(server.gitea_container, dump_cmd, user='git')
    if exit_code != 0:
        raise Exception(f'gitea dump failed (exit={exit_code})\n{out[:500]}')
...
def _backup_remote(server, backup):
    ...
    remote_tmp = f'/tmp/{backup.filename}'
    container_tmp = f'/tmp/{backup.filename}'

    dump_cmd = (
        f'docker exec -u git {server.gitea_container} '
        f'gitea dump -c /data/gitea/conf/app.ini -f "{container_tmp}"'
    )
    ...
    if exit_code != 0:
        raise Exception(f'docker cp failed (exit={exit_code})\nstdout: {out[:500]}\nstderr: {err[:500]}')
